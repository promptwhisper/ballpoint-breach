...
export async function saveBattleCard(dataUrl: string, browserWindow: XhsWindow = window): Promise<SaveBattleCardResult> {
  ...
  const filePath = miniTool.writeTempFile ? (await miniTool.writeTempFile({ data: dataUrl })).filePath : dataUrl;
...
export function saveBattleCardError(error: unknown): string {
  ...
  if (/deny|denied|auth|permission|权限/i.test(message)) return '未获得相册权限，请在系统设置中允许后重试。';
