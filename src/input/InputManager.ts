...
export class InputManager {
  ...
  setTouchSettings(sensitivity: number, fireMode: 'screen' | 'button'): void {
    if (this.fireMode !== fireMode) this.clearHeld();
    this.touchSensitivity = Number.isFinite(sensitivity) ? Math.max(0.5, Math.min(4, sensitivity)) : 3.5;
