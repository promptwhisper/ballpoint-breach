...
test('settings default to faster touch look and right-screen shooting', () => {
  assert.deepEqual(normalizeSettings(null), { sensitivity: 1.8, fireMode: 'screen', soundEnabled: true });
});
test('stored settings reject malformed values and bound sensitivity', () => {
  assert.equal(normalizeSettings({ sensitivity: NaN }).sensitivity, 1.8);
